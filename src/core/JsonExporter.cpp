...
static std::optional<fs::path> defaultGeneratedPath(const PBRTextureSet& ts, PBRTextureSlot slot)
{
    ...
    case PBRTextureSlot::Fuzz:
        return (ts.features.fuzz && hasExportedSlot(ts, PBRTextureSlot::Fuzz))
            ? std::optional<fs::path>(defaultBase)
            : std::nullopt;
    case PBRTextureSlot::Subsurface:
        ...
        return (ts.features.subsurface || ts.features.subsurfaceFoliage)
            ? std::optional<fs::path>(defaultBase)
            : std::nullopt;
...
static void setExplicitSlotIfNeeded(json& entry,
                                    const PBRTextureSet& ts,
                                    PBRTextureSlot slot,
                                    const char* slotName)
{
...
static json serializeEntryToJson(const PBRTextureSet& ts)
{
    ...
    if (ts.features.multilayer || ts.features.coatNormal) {
        ...
        if ((ts.features.coatNormal || (ts.features.fuzz && hasExportedSlot(ts, PBRTextureSlot::Fuzz)))
            && !hasExportedSlot(ts, PBRTextureSlot::CoatNormalRoughness)
            && !hasExportedSlot(ts, PBRTextureSlot::Fuzz)) {
            entry["lock_cnr"] = true;
        }
    }
    ...
    if ((ts.features.subsurface || ts.features.subsurfaceFoliage || ts.features.coatDiffuse)
        && !hasExportedSlot(ts, PBRTextureSlot::Subsurface)
        && !hasExportedSlot(ts, PBRTextureSlot::CoatColor)) {
